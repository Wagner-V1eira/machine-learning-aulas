...
def test_accuracy_score():
    ...
    funcs = load_notebook_funcs("modules/03-classificacao/exercises/01_classification_metrics.ipynb")
    ...
    assert accuracy == 1.0, f"Acurácia deveria ser 1.0 para classificação perfeita, obtido: {accuracy}"
...
def test_f1_score():
    ...
    assert f1 == 1.0, f"F1-Score deveria ser 1.0 para classificação perfeita, obtido: {f1}"
    ...
    f1 = f1_score(y_true, y_pred)
    expected_f1 = 2 * (1.0 * 0.5) / (1.0 + 0.5)  # 2 * (precision * recall) / (precision + recall)
    assert abs(f1 - expected_f1) < 1e-10, f"F1-Score deveria ser {expected_f1:.3f}, obtido: {f1}"
...
def test_confusion_matrix():
    ...
    assert np.array_equal(cm, expected_cm), f"Matriz de confusão incorreta. Esperado:\n{expected_cm}\nObtido:\n{cm}"
...
def test_classification_report():
    ...
    assert set(report.keys()) == expected_keys, f"Métricas esperadas: {expected_keys}, obtidas: {set(report.keys())}"

    # Verificar valores para classificação perfeita
    assert report["accuracy"] == 1.0, f"Acurácia deveria ser 1.0, obtido: {report['accuracy']}"
    assert report["precision"] == 1.0, f"Precisão deveria ser 1.0, obtido: {report['precision']}"
    assert report["recall"] == 1.0, f"Recall deveria ser 1.0, obtido: {report['recall']}"
...
def test_precision_recall_per_class():
    ...
    for class_name in expected_classes:
        assert per_class[class_name]["precision"] == 1.0, f"Precisão da {class_name} deveria ser 1.0"
        assert per_class[class_name]["recall"] == 1.0, f"Recall da {class_name} deveria ser 1.0"
...
def test_edge_cases():
    ...
    assert precision == 0.0, f"Precisão deveria ser 0.0 quando não há positivos preditos, obtido: {precision}"
    assert recall == 0.0, f"Recall deveria ser 0.0 quando não há positivos preditos, obtido: {recall}"
