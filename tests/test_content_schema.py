...
def test_module_yaml_files_exist():
    ...
    for module_path in modules:
        ...
        assert (
            yaml_path.exists()
        ), f"Arquivo module.yaml não encontrado em {module_path}"
...
def test_module_yaml_schema():
    ...
    for yaml_path in module_yamls:
        ...
        # Verificar tipos
        assert isinstance(
            module_data["slug"], str
        ), f"'slug' deve ser string em {yaml_path}"
        assert isinstance(
            module_data["title"], str
        ), f"'title' deve ser string em {yaml_path}"
        assert isinstance(
            module_data["order"], int
        ), f"'order' deve ser int em {yaml_path}"
        assert isinstance(
            module_data["prerequisites"], list
        ), f"'prerequisites' deve ser lista em {yaml_path}"
        assert isinstance(
            module_data["outcomes"], list
        ), f"'outcomes' deve ser lista em {yaml_path}"
        assert isinstance(
            module_data["lessons"], list
        ), f"'lessons' deve ser lista em {yaml_path}"
        assert isinstance(
            module_data["exercises"], list
        ), f"'exercises' deve ser lista em {yaml_path}"
    ...
    for yaml_path in module_yamls:
        ...
        for lesson in module_data["lessons"]:
            ...
            for field in required_lesson_fields:
                assert (
                    field in lesson
                ), f"Campo '{field}' ausente em lição de {yaml_path}"

            # Verificar se notebook existe
            notebook_path = yaml_path.parent / lesson["notebook"]
            assert (
                notebook_path.exists()
            ), f"Notebook {lesson['notebook']} não encontrado para {yaml_path}"
    ...
    for yaml_path in module_yamls:
        ...
        for exercise in module_data["exercises"]:
            ...
            for field in required_exercise_fields:
                assert (
                    field in exercise
                ), f"Campo '{field}' ausente em exercício de {yaml_path}"
            ...
            assert (
                notebook_path.exists()
            ), f"Notebook {exercise['notebook']} não encontrado para {yaml_path}"
            assert (
                tests_path.exists()
            ), f"Arquivo de testes {exercise['tests']} não encontrado para {yaml_path}"
...
def test_module_order_consistency():
    ...
    ), f"Ordens devem ser sequenciais a partir de 1, encontradas: {orders}"
    ...
    def test_prerequisites_exist(self):
        ...
        for yaml_path in module_yamls:
            ...
            for prereq in module_data["prerequisites"]:
                assert (
                    prereq in module_slugs
                ), f"Pré-requisito '{prereq}' não encontrado para {module_data['slug']}"
